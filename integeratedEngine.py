...
async def fetch_orders_from_api(token: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Since the API returns only the current user's orders, we'll extract the products
        purchased_product_ids = []

        for order in orders_data.get("data", []):
            # Extract product IDs from this user's orders
            for item in order.get("items", []):
                purchased_product_ids.append(item.get("product_id"))
...
async def initialize_data_from_api(token: str):
    ...
    try:
        ...
        purchased_product_ids = await fetch_orders_from_api(token)
